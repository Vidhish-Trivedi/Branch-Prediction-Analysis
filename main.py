# Code Written By: Vidhish Trivedi, July 2022.
...
len_trace_actual = len(trace_actual)
cnt_alwaysT = AlwaysT(trace_actual)
chosen_policy = 0

while(chosen_policy not in [1, 2, 3]):
    print()
...
if(chosen_policy == 1):  # For QUESTION-2(a).
    ...
    print("Number of branches taken (correct predictions):", cnt_alwaysT)
    print("Misprediction Rate (in percent):", (100)*((len_trace_actual - (cnt_alwaysT/len_trace_actual))))

elif(chosen_policy == 2):  # For QUESTION-2(a).
    # Count of branches not taken = total branches - count of branches taken.
    print("Using an always not taken prediction policy:")
    print("Number of branches not taken (correct predictions):", len_trace_actual - cnt_alwaysT)
    print("Misprediction Rate (in percent):", (100)*(cnt_alwaysT/len_trace_actual))
...
if(chosen_policy == 3):
    ...
    parr["Mis-prediction rate"] = [(100)*(cnt_alwaysT/len_trace_actual), (100)*((len_trace_actual - cnt_alwaysT)/len_trace_actual)]
